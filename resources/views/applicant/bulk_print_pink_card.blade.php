...
<div class="sixteen wide column center aligned">
	...
	<div class="ui attached fluid segment" style="min-height: 300px;">
		...
		<div style="overflow: auto;">
			...
			<form id="clear_ids_form" action="{{ url('pink_card/bulk_print_clear') }}" method="POST" style="display: none;">
				{{ csrf_field() }}
			</form>
		@endif
			...
			<div class="fields">
				...
				<div class="twelve wide field">
					<table class="ui celled striped center aligned selectable table">
						...
						<tbody id="to_prints">
							@if(session()->has('pink_card_print_ids'))
								@foreach(session('pink_card_print_ids') as $id)
									<tr data-id="{{ $id }}">
										<input type="hidden" name="ids[]" value="{{ $id }}">

										<td>
											{{ $pink_health_certificates->where('pink_health_certificate_id', $id)->first()->applicant->formatName() }}
										</td>

										<td>
											{{ $pink_health_certificates->where('pink_health_certificate_id', $id)->first()->registration_number }}
										</td>
...
@section('sub_custom_js')
	<script>
		var to_pink_card_print_ids = {!! json_encode(session()->get('pink_card_print_ids', [])) !!};
	</script>

	<script src="{{ mix('/js/bulk_print_pink_card.js') }}"></script>
@endsection